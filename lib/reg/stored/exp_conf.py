...
def grouped_exp_dic():
    ...
    def deb_exp(name, c=['feeder', 'gut'], dur=5.0, enrichment=None, **kwargs):
        if enrichment is None :
            enrichment = enr_dict(proc=['spatial'])
    ...
    d={
        ...
        'foraging': {
            ...
            'MB_patch_grid': food_exp('patch_grid', l=lgs(models=['MB_untrained', 'MB_trained'], N=3)),
            'noMB_patch_grid': food_exp('patch_grid', l=lgs(models=['noMB_untrained', 'noMB_trained'], N=10)),
