...
def build_table_window(data, pars_dict, title, return_layout=False):
    ...
    layout = [[sg.Text(' ', **kwargs2)] + [sg.Text(p, key=p, enable_events=True, **text_kwargs2) for p in pars]] + \
             ...
             [[sg.Button('Add', **b_kws), sg.Button('Remove', **b_kws),
               sg.Button('Ok', **b_kws), sg.Button('Cancel', **b_kws)]]
...
def named_bool_button(name, state, toggle_name=None):
    if toggle_name is None:
        toggle_name = name
    l = [sg.Text(f'{name} :', **t_kws), bool_button(toggle_name, state)]
    return l
...
def named_list_layout(text, key, choices, readonly=True, enable_events=True):
    l = [sg.Text(text, **header_kwargs),
         sg.Combo(choices, key=key, enable_events=enable_events, readonly=readonly, **t_kws)]
    return l
...
class ButtonGraphList(GraphList):
    def __init__(self, **kwargs):
        ...
        l = [sg.Button('Graph args', **b_kws, k=f'{self.name}_FIG_ARGS'),
             sg.Button('Draw', **b_kws, k=self.draw_key),
             sg.Button('Save', **b_kws, k=f'{self.name}_SAVE_FIG')]
