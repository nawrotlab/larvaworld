...
def init_pars():
    ...
    d = {
        ...
        'odorscape': {'odorscape': {'t': str, 'v': 'Gaussian', 'vs': ['Gaussian', 'Diffusion'], 'vfunc': param.Selector,
                                   'k': 'odorscape_mod',
        ...
        'windscape': {
        'wind_direction': {'t': float, 'v': np.pi, 'min': 0.0, 'max': 2 * np.pi, 'dv': 0.1, 'vfunc': param.Number,
        ...
        'space_search': {
        'pars': {'t': List[str], 'h': 'The parameters for space search.', 'k': 'ss.pars', 'vfunc': param.List},
        ...
        'stride_dst_std': {'v': 0.033, 'lim': (0.0, 1.0), 'aux_vs': ['sample'], 'disp': 'std', 'k': 'sstr_d_std',
                           'vfunc': param.Number,
        ...
        'initial_amp': {'lim': (0.0, 2.0), 'disp': 'initial', 'combo': 'amplitude', 'k': 'A_C0',
                        'label': 'initial crawler amplitude','vfunc': param.Number,'dv': 0.1, 'symbol': subsup('A', 'C','0'),
                        'h': 'The initial amplitude of the CRAWLER-generated forward velocity if this is hardcoded (e.g. constant waveform).'},
        'noise': {'v': 0.0, 'lim': (0.0, 1.0), 'dv': 0.01, 'disp': 'noise', 'combo': 'amplitude', 'k': 'A_Cnoise',
                  'vfunc': param.Magnitude,'symbol': subsup('A', 'C','noise'),'label': 'crawler output noise',
                  'h': 'The intrinsic output noise of the CRAWLER-generated forward velocity.'},
        'max_vel_phase': {'v': 3.6, 'lim': (0.0, 2 * np.pi), 'label': 'max velocity phase', 'k': 'phi_v_max',
        ...
        'windsensor': {
        'weights': {
            'hunch_lin': {'v': 10.0, 'lim': (-100.0, 100.0), 'disp': 'HUNCH->CRAWLER', 'vfunc': param.Number,
        ...
        'feeder': {
        'freq_range': {'t': Tuple[float], 'v': (1.0, 3.0), 'lim': (0.0, 4.0), 'disp': 'range', 'combo': 'frequency',
                       'vfunc': param.Range,
                       'h': 'The frequency range of the repetitive feeding behavior.'},
        'initial_freq': {'v': 2.0,'lim': (0.0, 4.0), 'disp': 'initial', 'combo': 'frequency', 'vfunc': param.Number,
        ...
        'phasic_interference': {
        'max_attenuation_phase': {'v': 3.4, 'max': 2 * np.pi, 'label': 'max relief phase', 'lim': (0.0, 2 * np.pi),
                                  'vfunc': param.Number,
        ...
        'suppression_mode': {'t': str, 'v': 'amplitude', 'vs': ['amplitude', 'oscillation', 'both'], 'k': 'IF_target',
                             'vfunc': param.Selector,
        ...
        'attenuation_max': {'v': 0.31, 'max': 1.0, 'lim': (0.0, 1.0), 'label': 'suppression relief coefficient.',
                            'vfunc': param.Magnitude,
    ...
    d['neural_turner'] = {
        ...
        'activation_range': {'t': Tuple[float], 'v': (10.0, 40.0), 'lim': (0.0,100.0), 'dv': 0.1, 'disp': 'range','k' : 'I_T_r',
    ...
    d['body'] = {'initial_length': {'v': 0.004, 'lim': (0.0,0.01),'dv': 0.0001, 'aux_vs': ['sample'], 'disp': 'initial',
                 ...
                 'length_std': {'v': 0.0, 'lim': (0.0,0.001), 'dv': 0.0001, 'u': ureg.m, 'aux_vs': ['sample'],'vfunc': param.Number, 'disp': 'std','k' : 'l_std',
    ...
    d['intermitter'] = {
        ...
        'EEB_decay': {'v': 1.0,  'lim': (0.0,2.0),'vfunc': param.Number,'symbol': sub('c', 'EEB'),'k': 'c_EEB',
        ...
        'crawl_freq': {'v': 1.43, 'lim': (0.5,2.5),'k': 'f_C', 'dv': 0.01,'u': ureg.Hz,'symbol': sub('f', 'C'), 'disp': 'crawling frequency','vfunc': param.Number,
                       'h': 'The default frequency of the CRAWLER oscillator when simulating offline.'},
        'feed_freq': {'v': 2.0, 'lim': (0.5,4.0), 'dv': 0.01,'k': 'f_F','u': ureg.Hz,'symbol': sub('f', 'F'), 'disp': 'feeding frequency','vfunc': param.Number,
                      'h': 'The default frequency of the FEEDER oscillator when simulating offline.'},
        'feeder_reoccurence_rate': { 'lim': (0.0,1.0), 'disp': 'feed reoccurence','symbol': sub('r', 'F'),'vfunc': param.Magnitude,
