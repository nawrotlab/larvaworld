...
def pref_exp(name, dur=5.0, c=['olfactor'], enrichment=PI,  as_entry=True,**kwargs,):
    e = exp(name, sim={'duration': dur}, c=c, enrichment=enrichment, **kwargs)
    return {name: e} if as_entry else e
...
exp_dict = {
    ...
    **pref_exp('odor_pref_test_on_food'),
     'odor_pref_train_short': pref_exp('odor_pref_train', dur=1.56, c=['olfactor', 'memory'],life_params='odor_preference_short', as_entry=False),
