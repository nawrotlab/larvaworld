    ...
    def __init__(self):
        ...
        def init_mods():
            d = dNl.NestDict({
                'bout_distro': {
                    ...
                    'alpha': {'lim': (-1000.0, 1000.0), 'disp': 'alpha', 'combo': 'distro', 'vfunc': param.Number,
                              'h': 'The "alpha" argument for constructing the distribution.'},
                    'beta': {'lim': (-1000.0, 1000.0), 'disp': 'beta', 'combo': 'distro', 'vfunc': param.Number,
                ...
                'crawler': {
                    'waveform': {'t': str, 'v': 'realistic', 'k': 'Cr_mod',
                                 ...
                                 'vfunc': param.Selector, 'symbol': subsup('A', 'C', 'mode'),
                    ...
                    'stride_dst_mean': {'v': 0.224, 'lim': (0.0, 1.0), 'dv': 0.01, 'aux_vs': ['sample'], 'disp': 'mean',
                                        'k': 'sstr_d_mu', 'vfunc': param.Number,
                    ...
                    'initial_amp': {'lim': (0.0, 2.0), 'disp': 'initial', 'combo': 'amplitude', 'k': 'A_C0',
                                    'label': 'initial crawler amplitude', 'vfunc': param.Number, 'dv': 0.1,
                                    'symbol': subsup('A', 'C', '0'),
                    ...
                    'noise': {'v': 0.0, 'lim': (0.0, 1.0), 'dv': 0.01, 'disp': 'noise', 'combo': 'amplitude',
                              'k': 'A_Cnoise',
                              'vfunc': param.Magnitude, 'symbol': subsup('A', 'C', 'noise'),
                ...
                'toucher': {
                    ...
                    'input_noise': {'v': 0.0, 'lim': (0.0, 1.0), 'h': 'The intrinsic noise of the sensory input.',
                                    'vfunc': param.Magnitude},
                    'decay_coef': {'v': 0.1, 'lim': (0.0, 2.0), 'vfunc': param.Number, 'label': 'tactile decay coef',
                    ...
                    'brute_force': {**bF, 'h': 'Whether to apply direct rule-based modulation on locomotion or not.'},
                    'initial_gain': {'v': 40.0, 'vfunc': param.Number, 'lim': (-100.0, 100.0),
                                     'label': 'tactile sensitivity coef', 'symbol': sub('G', 'T'), 'k': 'G_T',
                                     'h': 'The initial gain of the tactile sensor.'},
                    'touch_sensors': {'t': List[int], 'lim': (0, 8), 'k': 'sens_touch', 'vfunc': param.List,
                ...
                'feeder': {
                    ...
                    'feed_radius': {'v': 0.1, 'lim': (0.1, 10.0), 'vfunc': param.Number, 'symbol': sub('rad', 'F'),
                                    'label': 'feeding radius', 'k': 'rad_F',
                                    'h': 'The radius around the mouth in which food is consumable as a fraction of the body length.'},
                    'V_bite': {'v': 0.0005, 'lim': (0.0001, 0.01), 'dv': 0.0001, 'vfunc': param.Number,
                ...
                'memory': {
                    'modality': {'t': str, 'v': 'olfaction', 'vs': ['olfaction', 'touch'], 'vfunc': param.Selector,
                    ...
                    'epsilon': {'v': 0.3, 'max': 1.0, 'vfunc': param.Number,
                ...
                'base_interference': {
                    ...
                    'attenuation': {'v': 1.0, 'max': 1.0, 'lim': (0.0, 1.0), 'label': 'suppression coefficient',
                                    'symbol': '$c_{CT}^{0}$', 'k': 'c_CT0', 'vfunc': param.Magnitude,
                                    'h': 'CRAWLER:TURNER baseline suppression coefficient'},
                    'attenuation_max': {'v': 0.31, 'max': 1.0, 'lim': (0.0, 1.0),
            ...
            d['neural_turner'] = {
                ...
                'm': {'v': 100, 'min': 50, 'max': 200, 'label': 'maximum spike-rate', 'symbol': '$SR_{max}$',
                      'u_name': None,
                      'vfunc': param.Integer,
            ...
            d['base_turner'] = {
                ...
                'noise': {'v': 0.0, 'max': 1.0, 'disp': 'noise', 'combo': 'amplitude', 'k': 'A_Tnoise',
                          'vfunc': param.Magnitude,
                          'h': 'The intrinsic output noise of the TURNER activity amplitude.'},
                'activation_noise': {'v': 0.0, 'max': 1.0, 'disp': 'noise', 'combo': 'activation', 'k': 'I_Tnoise',
                                     'vfunc': param.Magnitude,
            ...
            d['intermitter'] = {
                ...
                'EEB': {'v': 0.0, 'max': 1.0, 'vfunc': param.Magnitude, 'symbol': 'EEB', 'k': 'EEB',
                        'h': 'The baseline exploitation-exploration balance. 0 means only exploitation, 1 only exploration.'},
                'EEB_decay': {'v': 1.0, 'lim': (0.0, 2.0), 'vfunc': param.Number, 'symbol': sub('c', 'EEB'),
                              'k': 'c_EEB',
                ...
                'crawl_freq': {'v': 1.43, 'lim': (0.5, 2.5), 'k': 'f_C', 'dv': 0.01, 'u': ureg.Hz,
                               'symbol': sub('f', 'C'),
                               'disp': 'crawling frequency', 'vfunc': param.Number,
                ...
                'feed_freq': {'v': 2.0, 'lim': (0.5, 4.0), 'dv': 0.01, 'k': 'f_F', 'u': ureg.Hz,
                              'symbol': sub('f', 'F'),
                              'disp': 'feeding frequency', 'vfunc': param.Number,
            ...
            d['gut'] = {
                ...
                'J_g_per_cm2': {'v': 10 ** -2 / (24 * 60 * 60), 'min': 0.0, 'disp': 'digestion secretion rate',
                                'vfunc': param.Number, 'symbol': 'J_g_per_cm2', 'k': 'J_g_per_cm2',
                                'h': 'Secretion rate of enzyme per unit of gut surface per second.'},
                'k_g': {'v': 1.0, 'min': 0.0, 'disp': 'digestion decay rate', 'vfunc': param.Number, 'symbol': 'k_g',
                        'k': 'k_g',
                        'h': 'Decay rate of digestive enzyme.'},
                'k_dig': {'v': 1.0, 'min': 0.0, 'disp': 'digestion rate', 'vfunc': param.Number, 'symbol': 'k_dig',
                          'k': 'k_dig',
                ...
                'k_c': {'v': 1.0, 'min': 0.0, 'vfunc': param.Number, 'disp': 'carrier release rate', 'symbol': 'k_c',
                        'k': 'gut_k_c',
                        'h': 'Release rate of carrier enzymes.'},
                'k_abs': {'v': 1.0, 'min': 0.0, 'vfunc': param.Number, 'disp': 'absorption rate', 'symbol': 'k_abs',
                          'k': 'gut_k_abs',
            ...
            d['DEB'] = {'species': {'t': str, 'v': 'default', 'vs': ['default', 'rover', 'sitter'], 'disp': 'phenotype',
                        ...
                        'hunger_gain': {'v': 0.0, 'max': 1.0, 'vfunc': param.Magnitude, 'symbol': sub('G', 'hunger'),
                        ...
                        'DEB_dt': {'lim': (0.0, 1000.0), 'disp': 'DEB timestep (sec)', 'vfunc': param.Number,
                                   'symbol': sub('dt', 'DEB'),
                                   'k': 'DEB_dt',
        ...
        def init_batch_pars():
            d = dNl.NestDict({
                ...
                'batch_methods': {
                    'run': {'t': str, 'v': 'default', 'vfunc': param.Selector,
                    ...
                    'final': {'t': str, 'v': 'null', 'vfunc': param.Selector,
                ...
                'space_search_par': {
                    'range': {'t': Tuple[float], 'max': 100.0, 'min': -100.0, 'dv': 1.0, 'vfunc': param.Range,
                              'k': 'ss.range',
        ...
        def init_enr_pars():
            ...
            d['preprocessing'] = {
                'rescale_by': {'max': 10.0, 'vfunc': param.Number,
        ...
        def init_distpars():
            ...
            d['substrate_composition'] = {
                n: {'v': 0.0, 'max': 10.0, 'h': f'{n} density in g/cm**3.', 'vfunc': param.Number} for
                n in
                ['glucose', 'dextrose', 'saccharose', 'yeast', 'agar', 'cornmeal']}

            d['substrate'] = {
                'type': {'t': str, 'v': 'standard', 'vs': list(substrate_dict.keys()), 'vfunc': param.Selector,
            ...
            d['food'] = {
                'radius': {'v': 0.003, 'max': 0.1, 'dv': 0.001, 'vfunc': param.Number,
            ...
            d['food_grid'] = {
                'unique_id': {'t': str, 'v': 'Food_grid', 'vfunc': param.String, 'disp': 'ID',
                ...
                'distribution': {'t': str, 'v': 'uniform', 'vfunc': param.Selector, 'vs': ['uniform'],
                                 'h': 'The distribution of food in the grid.'},
                'default_color': {'t': str, 'v': 'green', 'vfunc': param.Color, 'disp': 'color',
                                  'h': 'The default color of the food grid.'},
                **d['substrate']
            }
            ...
            d['life_history'] = {
                'age': {'v': 0.0, 'max': 250.0, 'vfunc': param.Number, 'dv': 1.0,
            ...
            d['spatial_distro'] = {
                'mode': {'t': str, 'vfunc': param.Selector, 'v': 'normal', 'vs': ['normal', 'periphery', 'uniform'],
                         'disp': 'placing',
                ...
                'N': {'t': int, 'vfunc': param.Integer, 'v': 10, 'max': 1000,
            ...
            d['larva_distro'] = {
                **d['spatial_distro'],
                'orientation_range': {'t': Tuple[float], 'vfunc': param.Range, 'v': (0.0, 360.0), 'min': 0.0,
                                      'max': 360.0,
                                      'dv': 1.0,
                                      'disp': 'heading',
            ...
            d['LarvaGroup'] = {
                ...
                'default_color': {'t': str, 'vfunc': param.Color, 'v': 'black', 'disp': 'color',
                                  'h': 'The default color of the larva group.'},
                'imitation': bF,
            ...
            d['source'] = {
                **d['agent'],
                'default_color': {'t': str, 'vfunc': param.Color, 'v': 'green', 'disp': 'color',
                                  'h': 'The default color of the source.'},
                'pos': d['xy'],
                **d['food'],
                'odor': d['odor']
            }
            ...
            d['SourceGroup'] = {
                ...
                'default_color': {'t': str, 'vfunc': param.Color, 'v': 'green', 'disp': 'color',
                                  'h': 'The default color of the source group.'},
                **d['food'],
                'odor': d['odor'],
            ...
            d['Border'] = {
                'default_color': {'t': str, 'vfunc': param.Color, 'v': 'black', 'disp': 'color',
                                  'h': 'The default color of the border.'},
        ...
        def init_pars0():
            d = dNl.NestDict({
                ...
                'odorscape': {
                    'odorscape': {'t': str, 'v': 'Gaussian', 'vs': ['Gaussian', 'Diffusion'], 'vfunc': param.Selector,
                                  'k': 'odorscape_mod',
                                  'h': 'The algorithm used for odorscape generation.'},
                    'grid_dims': {'t': Tuple[int], 'v': (51, 51), 'min': 10, 'max': 100, 'vfunc': param.Tuple,
                                  'k': 'grid_dims',
                                  'h': 'The odorscape grid resolution.'},
                    'evap_const': {'max': 1.0, 'k': 'c_evap',
                                   'h': 'The evaporation constant of the diffusion algorithm.',
                                   'vfunc': param.Magnitude},
                    'gaussian_sigma': {'t': Tuple[float], 'lim': (0.0, 1.0), 'max': 1.0, 'vfunc': param.NumericTuple,
                                       'k': 'gau_sigma',
                ...
                'air_puff': {
                    ...
                    'direction': {'v': 0.0, 'max': 100.0, 'h': 'The directions of the air puff in radians.',
                                  'vfunc': param.Number},
                    'start_time': {'v': 0.0, 'max': 10000.0, 'dv': 1.0,
                                   'h': 'The starting time of the air-puff in seconds.',
                                   'vfunc': param.Number},
                    'N': {'t': int, 'max': 10000, 'lim': (0, 10000),
                ...
                'windscape': {
                    'wind_direction': {'t': float, 'v': np.pi, 'min': 0.0, 'max': 2 * np.pi, 'dv': 0.1,
                                       'vfunc': param.Number,
                ...
                'arena': {
                    'arena_dims': {'t': Tuple[float], 'v': (0.1, 0.1), 'max': 1.0, 'dv': 0.01, 'disp': 'X,Y (m)',
                ...
                'sim_params': {
                    ...
                    'path': {'t': str, 'h': 'The relative path to save the simulation dataset.', 'k': 'path',
                             'vfunc': param.Foldername},
                    'duration': {'max': 100.0, 'h': 'The duration of the simulation in minutes.', 'vfunc': param.Number,
                                 'k': 't'},
        ...
        def buildInitDict():
            ...
            d['tracker'] = {
                'resolution': {
                    'fr': {'v': 10.0, 'max': 100.0, 'vfunc': param.Number, 'disp': 'framerate (Hz)',
                ...
                'filesystem': {
                    ...
                    'folder': {
                        'pref': {'t': str, 'vfunc': param.String, 'h': 'A prefix for detecting a raw-data folder.'},
                    ...
                    'file': {'pref': {'t': str, 'vfunc': param.String, 'h': 'A prefix for detecting a raw-data file.'},
                             ...
                             'sep': {'t': str, 'vfunc': param.String,
                                     'h': 'A separator for detecting a raw-data file.'}}
                },
            ...
            d['ga_select_kws'] = {
                'Nagents': {'t': int, 'vfunc': param.Integer, 'v': 30, 'min': 2, 'max': 1000,
                            'h': 'Number of agents per generation', 'k': 'N'},
                'Nelits': {'t': int, 'vfunc': param.Integer, 'v': 3, 'min': 0, 'max': 1000,
                           'h': 'Number of elite agents preserved per generation', 'k': 'Nel'},
                'Ngenerations': {'t': int, 'vfunc': param.Integer, 'max': 1000, 'h': 'Number of generations to run',
                                 'k': 'Ngen'},
            ...
            d['ga_build_kws'] = {
                ...
                'bestConfID': {'t': str, 'vfunc': param.String,
            ...
            d['obstacle_avoidance'] = {
                'sensor_delta_direction': {'v': 0.4, 'vfunc': param.Number, 'dv': 0.01, 'min': 0.2, 'max': 1.2,
                                           'h': 'Sensor delta_direction'},
                'sensor_saturation_value': {'t': int, 'vfunc': param.Integer, 'v': 40, 'min': 0, 'max': 200,
                                            'h': 'Sensor saturation value'},
                'obstacle_sensor_error': {'v': 0.35, 'vfunc': param.Number, 'dv': 0.01, 'max': 1.0,
                                          'h': 'Proximity sensor error'},
                'sensor_max_distance': {'v': 0.9, 'vfunc': param.Number, 'dv': 0.01, 'min': 0.1, 'max': 1.5,
                                        'h': 'Sensor max_distance'},
                'motor_ctrl_coefficient': {'t': int, 'vfunc': param.Integer, 'v': 8770, 'max': 10000,
                                           'h': 'Motor ctrl_coefficient'},
                'motor_ctrl_min_actuator_value': {'t': int, 'vfunc': param.Integer, 'v': 35, 'min': 0, 'max': 50,
            ...
            d['eval_conf'] = {
                ...
                'offline': {**bF, 'h': 'Whether to run a full LarvaworldSim environment', 'k': 'offline'},
                'N': {'t': int, 'vfunc': param.Integer, 'v': 5, 'min': 2, 'max': 1000,
