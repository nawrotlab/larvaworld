...
class DistributionRegistry:
    def __init__(self,load=False, save=False):
        self.path =  reg.Path['DistDict']
        if not load:
            self.dict = self.build()
            if save:
                dNl.save_dict(self.dict, self.path)
        else:
            self.dict = dNl.load_dict(self.path)


    def get_dist(self, k, k0='intermitter', v=None, return_tabrows=False, return_all=False):
        d0=self.dict
        dict0 = {
