...
if test_single_puffs :
    ...
    puffs={i:preg.get_null('air_puff', duration=5, speed=50, direction=i/Npuffs*2*np.pi, start_time=5+10*i) for i in range(Npuffs)}
    wind_speed = 0.0
elif test_repetitive_puffs :
    media_name = 'repetitive_air-puffs'
    puffs= {'puff_group':preg.get_null('air_puff', duration=5, speed=50, direction=np.pi/4, start_time=5, N=Npuffs, interval=10.0)}
    wind_speed = 0.0
else :
    if test_direction:
