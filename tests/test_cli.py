...
def test_run_experiment_no_analysis_no_video():
    ...
    result = shell(f"{cli_entry_point} Exp dish -N 10 -duration 3.0")
    assert result.exit_code == 0


def test_run_experiment_with_analysis_no_video():
    """Run an experiment with analysis, without visualization on screen."""
    result = shell(f"{cli_entry_point} Exp chemorbit -N 10 -duration 3.0 -a")
    assert result.exit_code == 0
...
def test_run_experiment_with_analysis_with_video():
    ...
    result = shell(
        f"{cli_entry_point} Exp dispersion -N 10 -duration 3.0 -vis_mode video -a"
    )
    assert result.exit_code == 0


def test_run_experiment_no_analysis_with_video():
    """Run an experiment without analysis, with visualization on screen."""
    result = shell(
        f"{cli_entry_point} Exp dispersion -N 10 -duration 3.0 -vis_mode video"
    )
    assert result.exit_code == 0
...
def test_run_experiment_no_analysis_with_image():
    """Run an experiment without analysis, with visualization on screen."""
    result = shell(
        f"{cli_entry_point} Exp dispersion -N 10 -duration 3.0 -vis_mode image"
    )
    assert result.exit_code == 0


def xtest_run_ga_without_video():
    """Run a genetic algorithm experiment without visualization on screen."""
    result = shell(
        f"{cli_entry_point} Ga realism -refID exploration.30controls -N 20 -duration 0.5 -mID1 GA_test_loco"
    )
    assert result.exit_code == 0


def xtest_run_ga_with_video():
    """Run a genetic algorithm experiment with visualization on screen."""
    result = shell(
        f"{cli_entry_point} Ga realism -refID exploration.30controls -N 20 -duration 0.5 -mID1 GA_test_loco -vis_mode image"
    )
    assert result.exit_code == 0


def xtest_run_replay_on_dataset_by_ID():
...
def xtest_run_replay_on_dataset_by_dir():
...
def xtest_evaluation_run():
    """Perform an experiment evaluation run."""
    result = shell(
        f"{cli_entry_point} Eval -refID exploration.30controls -mIDs RE_NEU_PHI_DEF RE_SIN_PHI_DEF -N 5"
    )
    assert result.exit_code == 0
