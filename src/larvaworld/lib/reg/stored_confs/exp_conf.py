...
def grouped_exp_dic():
    ...
    d0={
        ...
        'dispersion_x4':{'env': 'arena_200mm', 'dur':3.0,
                         'l' : lgs(mIDs=['loco_default', 'Levy', 'nengo_explorer'],ids=['CoupledOsc', 'Levy', 'nengo'], N=5)
                         }
    }
    ...
    d={
        ...
        'odor_preference': {
            'PItest_off': pref_exp('PItest_off',env= 'CS_UCS_off_food', dur=3.0, l=lg(N=25, s=(0.005, 0.02), mID='RE_NEU_PHI_DEF_nav_x2')),
        ...
        'foraging': {
            ...
            'MB_patch_grid': food_exp('MB_patch_grid',env= 'patch_grid',c=['feeder', 'olfactor'], l=lgs(mIDs=['MB_untrained', 'MB_trained'], N=3)),
            'noMB_patch_grid': food_exp('noMB_patch_grid', env='patch_grid',c=['feeder', 'olfactor'], l=lgs(mIDs=['noMB_untrained', 'noMB_trained'], N=4)),
            'random_food': food_exp('random_food', env='random_food', c=['feeder', 'toucher'], l=lgs(mIDs=['RE_NEU_PHI_DEF_feeder', 'RL_forager'],
                                                                                  ids=['Orco', 'RL'], N=5,
                                                                                  mode='uniform',
                                                                                  shape='rectangular', s=0.04),
                                    enrichment=gen.EnrichConf(proc_keys=['spatial']).nestedConf, en=False),
            'uniform_food': food_exp('uniform_food',env= 'uniform_food', l=lg(mID='RE_NEU_PHI_DEF_feeder', N=5, s=0.005)),
        ...
        'zebrafish': {
            'prey_detection': exp('prey_detection',env= 'windy_blob_arena', l=lg(mID='zebrafish', N=4, s=(0.002, 0.005)),
                                  sim={'duration': 20.0})
        },

        'other': {
            'realistic_imitation': exp('realistic_imitation',env= 'dish', l=lg(mID='imitator', N=25), sim={'Box2D': True}, c=['midline', 'contour'])
