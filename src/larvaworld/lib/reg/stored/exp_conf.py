...
def grouped_exp_dic():
    ...
    def exp(id, env=None, l={}, en=False, sim={}, c=[], as_entry=False, **kwargs):
        if env is None :
            env=id
    ...
    def game_exp(id,  c=[], dur=20.0, **kwargs):
        return exp(id, sim={'duration': dur}, c=c, **kwargs)


    def deb_exp(id,  c=['feeder', 'gut'], dur=5.0, enrichment=None, **kwargs):
        if enrichment is None :
            enrichment = reg.par.enr_dict(proc=['spatial'])
        return exp(id,  sim={'duration': dur}, c=c, enrichment=enrichment, **kwargs)


    def simple_exp(id, dur=10.0, en=True, **kwargs):
        return exp(id, sim={'duration': dur}, en=en, **kwargs)


    def anemo_exp(id,  dur=5.0, c=['wind'], en=False, **kwargs):
        return exp(id,  sim={'duration': dur}, c=c, en=en,
                   enrichment=reg.par.enr_dict(proc=['spatial', 'angular', 'wind']), **kwargs)


    def thermo_exp(id, dur=10.0, c=['thermo'], en=False, enrichment=None, **kwargs):
        return exp(id, sim={'duration': dur}, c=c, en=en, enrichment=enrichment, **kwargs)


    def chemanemo_exp(id,  dur=5.0, c=['olfactor', 'wind'], en=False,
                      enrichment=reg.par.enr_dict(proc=['spatial', 'angular', 'source', 'wind'],
                                               anot=['bout_detection', 'source_attraction']), **kwargs):
        return exp(id,  sim={'duration': dur}, c=c, en=en, enrichment=enrichment, **kwargs)


    def pref_exp(id, dur=5.0, c=[], enrichment=reg.par.enr_dict(proc=['PI']), **kwargs):
        return exp(id, sim={'duration': dur}, c=c, enrichment=enrichment, **kwargs)
    ...
    d0={
        ...
        'dispersion': {'env': 'arena_200mm',
                       'l' : reg.lg(mID='SQ_NEU_PHI_DEF', N=25)},
    ...
    d={
        ...
        'foraging': {
            ...
            'random_food': food_exp('random_food', env='random_food', c=['feeder', 'toucher'], l=reg.lgs(mIDs=['Orco_forager', 'RL_forager'],
                                                                                  ids=['Orco', 'RL'], N=5,
                                                                                  mode='uniform',
                                                                                  shape='rectangular', s=0.04),
                                    enrichment=reg.par.enr_dict(proc=['spatial']), en=False),
            'uniform_food': food_exp('uniform_food',env= 'uniform_food', l=reg.lg(mID='Orco_forager', N=5, s=0.005)),
            'food_grid': food_exp('food_grid',env= 'food_grid', l=reg.lg(mID='Orco_forager', N=25)),
