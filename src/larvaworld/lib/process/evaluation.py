...
class GaEvaluation(Evaluation) :
    def __init__(self, fit_kws, **kwargs):
        ...
        self.fit_dict = GA_optimization(d=self.target, fit_kws=self.fit_kws)
...
def GA_optimization(d, fit_kws):
    ...
    def fit_func(s):
        ...
        for kfunc in func_global_dict.values():
            fit_dicts.update(kfunc(s))
        return fit_dicts
    return aux.AttrDict({'func': fit_func, 'keys': aux.unique_list(keys), 'func_arg': 's'})
