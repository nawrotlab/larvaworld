"""
Brain implementation backed by a Nengo spiking network.

Wires the behavioural modules into a Nengo model so that the oscillators and
the intermittency are realized by simulated neural populations rather than by
the analytical implementations.
"""
...
class NengoBrain(Network, Brain):
    """
    Brain whose rhythms are generated by a Nengo spiking network.

    Realizes the crawler, turner, feeder and intermitter as neural ensembles
    within one Nengo model, so that the behavioural oscillations emerge from
    simulated population dynamics instead of the analytical implementations.
    """
