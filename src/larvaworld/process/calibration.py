...
from larvaworld.process.spatial import comp_centroid
from larvaworld.process.annotation import detect_strides, process_epochs
...
def comp_stride_variation(d):
    ...
    if any([vv not in s.columns for vv in vels + [cvel]]):
        from larvaworld.process.spatial import comp_spatial
        comp_centroid(s, c, recompute=False)
        comp_spatial(s, e, c, mode='full')

    if any([vv not in s.columns for vv in lvels]):
        from larvaworld.process.spatial import comp_linear
        from larvaworld.process.angular import comp_orientations
        comp_orientations(s, e, c, mode='full')
        comp_linear(s, e, c, mode='full')

    if any([vv not in s.columns for vv in all_vels]):
        from larvaworld.process.spatial import scale_to_length
...
def comp_segmentation(d):
    ...
    if not set(avels).issubset(s.columns.values):
        from larvaworld.process.angular import comp_angles, comp_angular
