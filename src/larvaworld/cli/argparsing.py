...
def run_template(sim_mode, args, kw_dicts, kws):
    '''
...
def get_parser(sim_mode, parser=None):
    ...
    for k in ks:
        if k == 'e':
            p.add_argument('experiment', choices=list(reg.CONFTREE[sim_mode].keys()), help='The experiment mode')
...
def subparser_dict(p):
    ...
    for sp_key, (mks,ks) in dic.items() :
        sp = subparsers.add_parser(sp_key)
        # mks, ks = dic[sim_mode]
        mks.append('sim_params')
        ...
        for k in ks:
            if k == 'e':
                sp.add_argument('experiment', choices=list(reg.CONFTREE[sp_key].keys()), help='The experiment mode')
                # p.add_argument('experiment', choices=reg.stored.confIDs(sim_mode), help='The experiment mode')
            # elif k == 't':
            #     p.add_argument('-t', '--duration', type=float, help='The duration of the simulation in minutes')
            # elif k == 'Box2D':
