...
def get_valid_plots_for_datasets(
    ...
    for group_id, group_data in graphgroups.items():
        valid_fids: list[str] = []
        if "entries" not in group_data:
            continue
        for entry_id in group_data["entries"]:
            fid = entry_id
            if fid not in graph_registry.dict:
                continue
            try:
                graph_registry.run(
                    fid,
