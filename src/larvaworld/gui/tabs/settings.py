...
class SettingsTab(gui_aux.GuiTab):
    ...
    def used_keys(self):
        ...
        return util.flatten_list([list(k.values()) for k in list(d.values())])

    def single_control_layout(self, k, v, prefix=None, editable=True):
        k0 = f"{prefix} {k}" if prefix is not None else k
        l = [
            sg.T(k, **gui_aux.t_kws(14)),
            sg.In(
                default_text=v,
                key=self.control_k(k0),
                disabled=True,
    ...
    def single_control_collapsible(self, name, dic, editable=True, **kwargs):
        l = [
            self.single_control_layout(k, v, prefix=name, editable=editable)
    ...
    def build_controls_collapsible(self, c):
        ...
        b_reset = gui_aux.GraphButton(
            "Button_Burn",
            self.k_reset,
            tooltip="Reset all controls to the defaults. "
            "Restart Larvaworld after changing shortcuts.",
        )
        conf = reg.controls.load()
        cs = [
            self.single_control_collapsible(k, v, header_width=26, **kws)
            for k, v in conf["keys"].items()
        ]
        cs.append(
            self.single_control_collapsible(
                "mouse", conf["mouse"], editable=False, header_width=26, **kws
            )
        )
        l = []
        for cc in cs:
        ...
        c_controls = gui_aux.PadDict(
            "Controls",
            content=l,
            after_header=[b_reset],
            Ncols=3,
            header_width=90,
            **kws,
        )
