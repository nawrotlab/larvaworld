...
class EnvTab(gui_aux.GuiTab):
    ...
    def get(self, w, v, c, as_entry=False):
        return {
            "food_params": {
                n: c[n].get_dict(v, w) for n in [self.Sg, self.Su, "food_grid"]
            },
            **{
                n: c[n].get_dict(v, w)
                for n in [self.Bg, "arena", "odorscape", "windscape"]
    ...
    def build(self):
        ...
        c2 = gui_aux.PadDict(
            self.S, content=l1, header_width=34, dict_name=self.S.lower()
        )
        c.update(c2.get_subdicts())
        sl1 = gui_aux.SelectionList(
            tab=self,
            buttons=["save", "delete", "tree", "conf_tree"],
            disp=self.name,
            width=35,
            root_key="Env",
        )
        l = gui_aux.gui_cols(
            [[sl1, s7, s8, s4], [c2]], x_fracs=[0.25, 0.25], as_pane=True, pad=(10, 10)
        )
        self.layout = l
        return l, c, {}, {}
