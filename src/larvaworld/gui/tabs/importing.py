...
class ImportTab(gui_aux.GuiTab):
    ...
    def update(self, w, c, conf, id=None):
        path = conf["path"]
        w[f"BROWSE {self.raw_key}"].InitialFolder = f"{DATA_DIR}/{path}/raw"
        w[f"BROWSE {self.proc_key}"].InitialFolder = f"{DATA_DIR}/{path}/processed"
    ...
    def build(self):
        ...
        dl1 = gui_aux.DataList(
            ...
            tab=self,
            dict=d[kR],
            buttons=["import", "select_all", "remove", "change_ID", "browse"],
            raw=True,
            size=(50, 10),
        )
        dl2 = gui_aux.DataList(
            ...
            aux_cols=["N", "duration", "quality"],
            size=(55, 9),
        )

        pd2 = gui_aux.PadDict(
            "enrichment",
            background_color=self.Cenr,
            header_width=125,
            subconfs={
                "preprocessing": {"text_kws": gui_aux.t_kws(14)},
        ...
        pd1 = gui_aux.PadDict(
            "Tracker",
            background_color=self.Ctrac,
            text_kws=gui_aux.t_kws(8),
            header_width=22,
            subconfs={
                "filesystem": {"header_width": 20, "value_kws": gui_aux.t_kws(5)},
