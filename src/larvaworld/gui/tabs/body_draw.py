...
class DrawBodyTab(gui_aux.DrawTab):
    ...
    def build(self):
        ...
        sl = gui_aux.SelectionList(
            tab=self,
            disp="Body",
            buttons=["load", "save", "delete", "exec"],
            width=30,
            text_kws=gui_aux.t_kws(12),
        )
        ...
        c1 = gui_aux.PadDict(
            ...
            subconfs={
                self.P: {
                    ...
                    "indexing": True,
                    "group_by_N": 2,
                    "text_kws": {"text_color": self.Cdict[self.P]},
                },
                self.T: {
                    "Nspins": 8,
                    "group_by_N": 4,
                    "text_kws": {"text_color": self.Cdict[self.T]},
                },
                self.O: {"Nspins": 4, "text_kws": {"text_color": self.Cdict[self.O]}},
            },
            after_header=[
                gui_aux.GraphButton(
                    "Button_Burn", "RESET_BODY", tooltip="Reset to the initial body."
                ),
                gui_aux.GraphButton(
                    "Globe_Active",
                    "NEW_BODY",
        ...
        col2 = sg.Col(
            [g1.canvas.get_layout(as_pane=True, pad=(0, 10))[0]], **gui_aux.col_kws
        )
        l = [[col1, col2]]
    ...
    def update(self, w, c, conf, id):
        if conf[self.P] is not None:
            conf[self.P] = util.rearrange_contour(conf[self.P])
    ...
    def draw_points(self, conf, **kwargs):
        ...
        for i, p0 in enumerate(conf[self.P]):
            ...
            if ts is not None and i in ts:
                f = gg.draw_circle(
                    p, r * 1.8, line_width=6, line_color=self.Cdict[self.T]
                )
                dic[self.T][i] = {"fig": f, "pos": p}
            if os is not None and i in os:
                f = gg.draw_circle(
                    p, r * 2.6, line_width=5, line_color=self.Cdict[self.O]
                )
    ...
    def draw_segs(self, conf, **kwargs):
        dic = self.base_dict
        gg = self.graph
        base_seg_vertices = util.generate_seg_shapes(centered=False, **conf)
        for i in range(base_seg_vertices.shape[0]):
            ps = [
                self.scale_xy(base_seg_vertices[i] - np.array([0.5, 0.0]), reverse=True)
                for i in range(base_seg_vertices.shape[0])
            ]
            ps.append(ps[0])
