...
class SimTab(gui_aux.GuiTab):
    ...
    def build(self):
        ...
        ts = [
            sg.Tab(n, nl, key=f"{n}_SIM TAB")
            for n, nl in zip(["draw", "setup", "exec"], [l3, l1, l2])
        ]
        l_tabs = sg.TabGroup(
            [ts], key="ACTIVE_SIM_TAB", tab_location="topleft", **gui_aux.tab_kws
        )
        l = [[l0, l_tabs]]
    ...
    def build_RUN(self):
        ...
        g1 = gui_aux.GraphList(
            ...
            list_size=(gui_aux.w_list, 15),
        )
        dl1 = gui_aux.DataList(
            kA,
            dict=d[kA],
            tab=self,
            buttons=["select_all", "stop"],
            disp="Active simulations",
            size=(gui_aux.w_list, 6),
        )
        dl2 = gui_aux.DataList(
            kS,
            dict=d[kS],
            tab=self,
            buttons=["select_all", "remove"],
            disp="Completed simulations",
            size=(gui_aux.w_list, 6),
        )
        l = gui_aux.gui_cols(
            cols=[[g1.canvas], [dl1, dl2, g1]],
            x_fracs=[0.53, 0.25],
            as_pane=True,
            pad=(10, 10),
        )
    ...
    def build_conf(self):
        ...
        sl3 = gui_aux.SelectionList(
            tab=self,
            buttons=["load", "save", "delete", "exec", "tree", "conf_tree"],
            progress=True,
            root_key="Exp",
            sublists={"env_params": sl1, "larva_groups": s1},
            text_kws=gui_aux.t_kws(10),
            width=28,
        )
        ...
        c2 = gui_aux.PadDict(
            "output", text_kws=gui_aux.t_kws(7), Ncols=2, header_width=30, **kws
        )
        ...
        c.update(**tab1_c)
        l0 = gui_aux.gui_col(
            [sl3, s1, c["arena"]], x_frac=0.2, as_pane=True, pad=(10, 10)
        )
    ...
    def update(self, w, c, conf, id):
        ...
        c["output"].update(
            w, dict(zip(ks, [True if k in conf["collections"] else False for k in ks]))
        )
        sim = copy.deepcopy(conf["sim_params"])
        sim.update(
            {
                "sim_ID": f'{id}_{reg.config.next_idx(id=id, conftype="Exp")}',
                "path": f"single_runs/{id}",
            }
        )
        c["sim_params"].update(w, sim)
        c["trials"].update(w, reg.conf.Trial.getID(conf["trials"]))
        self.draw_tab.set_env_db(
            env=reg.conf.Env.getID(conf.env_params), lg=conf["larva_groups"]
        )
    ...
    def get(self, w, v, c, as_entry=True):
        conf = {
            ...
            "collections": [
                k for k in reg.parDB.output_keys if c["output"].get_dict(v, w)[k]
            ],
    ...
    def eval(self, e, v, w, c, d, g):
        ...
        if (
            self.base_list.progressbar is not None
            and e == self.base_list.progressbar.k_incomplete
        ):
            pass
