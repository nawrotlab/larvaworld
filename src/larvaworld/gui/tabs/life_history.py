...
class LifeTab(gui_aux.GuiTab):
    ...
    def build(self):
        ...
        sub = gui_aux.CollapsibleDict(
            "substrate",
            dict_name="substrate_composition",
            header_dict=substrate_dict,
            header_value="standard",
            state=True,
            value_kws=gui_aux.t_kws(8),
        )
        l1 = [
            [sg.T("Epoch start (hours) : ", **gui_aux.t_kws(24))],
            [
                sg.Slider(
                    range=(0, 150),
                    default_value=0,
        ...
        l2 = [
            [sg.T("Food quality : ", **gui_aux.t_kws(24))],
            [
                sg.Slider(
                    range=(0.0, 1.0),
                    default_value=1.0,
        ...
        after_header = [
            gui_aux.GraphButton("Button_Add", f"ADD {ep}", tooltip=f"Add a new {ep}."),
            gui_aux.GraphButton(
                "Button_Remove", f"REMOVE {ep}", tooltip=f"Remove an existing {ep}."
            ),
        ]
        content = [
            gui_aux.Table(
                headings=[self.s0, self.s1, "quality", "type"],
                col_widths=[5, 5, 6, 7],
                key=self.K,
                num_rows=8,
            )
        ]
        l_tab = gui_aux.Header(
            "Epochs",
            text=f"{ep.capitalize()}s (h)",
            text_kws=gui_aux.t_kws(18),
        ...
        g1 = gui_aux.GraphList(
            self.name,
            tab=self,
            fig_dict={m: plot_debs for m in deb_modes},
            default_values=["reserve"],
            canvas_size=r1_size,
            list_header="DEB parameters",
            auto_eval=False,
        )
        ...
        return {
            "epochs": {
                i: {
                    "start": r[0],
                    "stop": r[1],
                    "substrate": Substrate(type=r[3], quality=r[2]),
                }
    ...
    def eval(self, e, v, w, c, d, g):
        ...
        if e == self.graphlist_k:
            ...
            if q > 0:
                ...
                fig, save_to, filename = plot_debs(
                    deb_dicts=[D], mode=v[self.graphlist_k][0], return_fig=True
                )
