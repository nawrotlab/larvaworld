...
class DrawEnvTab(gui_aux.DrawTab):
    def __init__(self, canvas_size=(800, 800), **kwargs):
        ...
        self.Su, self.Sg, self.SD = (
            f"{self.S0}_units",
            f"{self.S0}_groups",
            f"{self.S0}_distro",
        )
        self.Lu, self.Lg, self.LD = (
            f"{self.L0}_units",
            f"{self.L0}_groups",
            f"{self.L0}_distro",
        )
    ...
    def add_agent_layout(self, o, color, c):
        ...
        l = [
            [
                sg.R(f"Add {o}", 1, k=o, enable_events=True, **gui_aux.t_kws(10)),
                *gui_aux.color_pick_layout(o, color),
            ],
            [
                sg.T("", **gui_aux.t_kws(2)),
                sg.R("single ID", k=U, **kws2),
                sg.In(o, k=Uid),
            ],
            [
                sg.T("", **gui_aux.t_kws(2)),
                sg.R("group ID", k=G, **kws2),
                sg.In(G, k=Gid),
            ],
        ]
        if o == self.S0:
    ...
    def build(self):
        ...
        lB = [
            [
                sg.R(f"Add {B0}", 1, k=B0, enable_events=True, **gui_aux.t_kws(10)),
                *gui_aux.color_pick_layout(B0, "black"),
            ],
            [
                sg.T("", **gui_aux.t_kws(2)),
                sg.T("id", **gui_aux.t_kws(5)),
                sg.In(B0, k=f"{B0}_id"),
            ],
            [
                sg.T("", **gui_aux.t_kws(2)),
                sg.T("width", **gui_aux.t_kws(5)),
                sg.Spin(
                    values=np.arange(0.1, 1000, 0.1).tolist(),
                    initial_value=0.001,
                    k=f"{B0}_width",
                ),
            ],
        ...
        col2 = sg.Col(
            [
                [sg.Pane([sg.Col(ll, **gui_aux.col_kws)], border_width=8, pad=(10, 10))]
                for ll in [lL, lB, lI]
            ]
        )
        col3 = sg.Col(
            [[sg.Pane([sg.Col(lS, **gui_aux.col_kws)], border_width=8, pad=(10, 10))]],
            **gui_aux.col_kws,
        )
        g1 = gui_aux.GraphList(
            self.name,
            tab=self,
            graph=True,
    ...
    def eval(self, e, v, w, c, d, g):
        ...
        if e == self.canvas_k:
            ...
            if not dic["dragging"]:
                ...
                dic["dragging"] = True
                dic["drag_figures"] = [
                    f for f in gg.get_figures_at_location((x, y)) if f != dic["arena"]
                ]
                dic["last_xy"] = x, y
            else:
            ...
            if None not in self.get_drag_ps():
                if v["-MOVE-"]:
                    ...
                    for fig in dic["drag_figures"]:
                        for k in list(db.keys()):
                            if fig in list(db[k]["figs"].keys()):
                                id = db[k]["figs"][fig]
                                w["out"].update(
                                    value=f"Item {id} moved by ({delta_X}, {delta_Y})"
                                )
                                ...
                                for f in figs:
                                    if k == self.Su:
                                        ...
                                        db[k]["items"][id]["points"] = [
                                            ...
                                            for X0, Y0 in db[k]["items"][id]["points"]
                                        ]
                    ...
                    if any([self.out_of_bounds(P, v, w, c) for P in [P1, P2]]):
                        ...
                        if v[S0] and not self.check_abort(
                            S0, w, v, db[self.Su]["items"], db[self.Sg]["items"]
                        ):
                            o = S0
                            U, G = f"{o}_unit", f"{o}_group"
                            Uid, Gid = f"{U}_id", f"{G}_id"
                            ...
                            if v[Uid] or (v[Gid] and dic["sample_fig"] is None):
                                ...
                                dic["prior_rect"] = self.draw_shape(
                                    shape=v[sh],
                                    p1=p1,
                                    p2=p2,
                                    line_color=color,
                                ...
                                dic["current"] = {
                                    v[Gid]: class_defaults(
                                        reg.gen.FoodGroup, **temp_dic
                                    )
                                }
                                dic["prior_rect"] = self.draw_shape(
                                    shape=v[f"{D}_shape"],
                        ...
                        elif v[L0] and not self.check_abort(
                            L0, w, v, db[self.Lu]["items"], db[self.Lg]["items"]
                        ):
                            o = S0
                            U, G = f"{o}_unit", f"{o}_group"
                            Uid, Gid = f"{U}_id", f"{G}_id"
                            ...
                            if v[G]:
                                ...
                                dic["current"] = {
                                    v[Gid]: class_defaults(
                                        reg.larvagroup.LarvaGroup, **temp_dic
                                    )
                                }
                                dic["prior_rect"] = self.draw_shape(
                                    shape=v[f"{D}_shape"],
                            ...
                            if id in list(db[self.Bg]["items"].keys()) or id == "":
                                info.update(
                                    value=f"{B} id {id} already exists or is empty"
                                )
                            else:
                                dic["current"] = {
                                    id: class_defaults(
            ...
            if dic["arena"] is None:
                return d, g
            P1, P2 = self.get_drag_ps(scaled=True)
            current, prior_rect, sample_pars = (
                dic["current"],
                dic["prior_rect"],
                dic["sample_pars"],
            )
            if v[B0] and current != {}:
                ...
                if v[U] and current != {}:
                    ...
                    if current == {}:
                        info.update(
                            value=f"Sample item for source group {id} detected."
    ...
    def draw_arena(self, v, w, c):
        ...
        kws = {"fill_color": "white", "line_color": "black"}
        if shape == "circular" and X is not None:
            arena = g.draw_circle(
                (int(W / 2), int(H / 2)), int(W / 2), line_width=5, **kws
            )
            s = W / X
        elif shape == "rectangular" and X is not None and Y is not None:
            if X >= Y:
                dif = (X - Y) / X
                arena = g.draw_rectangle(
                    (0, int(H * dif / 2)),
                    (W, H - int(H * dif / 2)),
                    line_width=5,
                    **kws,
                ...
                dif = (Y - X) / Y
                arena = g.draw_rectangle(
                    (int(W * dif / 2), 0), (W - int(W * dif / 2), H), **kws
                )
                s = H / Y
        self.base_dict["s"] = s
    ...
    def reset_arena(self, v, w, c):
        ...
        for id, ps in db[self.Bg]["items"].items():
            ...
            f = self.graph.draw_lines(
                points=points, color=ps["" "color"], width=int(ps["width"] * self.s)
            )
    ...
    def inspect_distro(
        ...
        **kwargs,
        ...
        Ps = generate_xy_distro(
            ...
            shape,
            N,
            loc=self.scale_xy(loc, reverse=True),
            scale=np.array(scale) * self.s,
        )
        group_figs = []
        for i, P0 in enumerate(Ps):
            if item == self.S0:
                temp = self.draw_source(P0, color, **kwargs)
            elif item == self.L0:
                if distribution is not None:
                    orientation_range = distribution["orientation_range"]
                temp = self.draw_larva(P0, color, orientation_range, **kwargs)
            group_figs.append(temp)
        return group_figs

    def draw_source(self, P0, color, amount, radius, **kwargs):
        fill_color = (color) if amount > 0 else None
        temp = self.graph.draw_circle(
            P0, radius * self.s, line_width=3, line_color=color, fill_color=fill_color
        )
        return temp

    def draw_larva(self, P0, color, orientation_range, **kwargs):
        xy0 = util.xy.body_contour() - np.array([0.5, 0.0])
        xy0 = util.rotate_points_around_point(
            xy0, random.uniform(*np.deg2rad(orientation_range)), origin=[0, 0]
        )
        xy0 = xy0 * self.s / 250 + np.array(P0)
        temp = self.graph.draw_polygon(
            xy0, line_width=3, line_color=color, fill_color=color
        )
        return temp
    ...
    def set_env_db(self, env=None, lg={}, store=True):
        if env is None:
            env = {
                self.Bg: {},
                "arena": class_defaults(reg.gen.Arena),
                "food_params": {self.Su: {}, self.Sg: {}, "food_grid": None},
            }
        items = [
            env[self.Bg],
            env["food_params"][self.Su],
            env["food_params"][self.Sg],
            {},
            lg,
        ]
        dic = {
            k: {"items": ii, "figs": {}}
            for k, ii in zip([self.Bg, self.Su, self.Sg, self.Lu, self.Lg], items)
        }
        if store:
