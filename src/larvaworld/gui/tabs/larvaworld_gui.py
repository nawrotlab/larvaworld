...
class LarvaworldGui:
    ...
    def __init__(self, tabs=None, add_terminal=True):
        ...
        # raise
        l_tabs, self.collapsibles, self.graph_lists, self.dicts, self.tabs = self.build(
            tabs
        )
        if add_terminal:
            ...
            layout = [
                [sg.Pane([sg.vtop(l_tabs), sg.vbottom(self.terminal)], handle_size=30)]
            ]
        else:
            self.terminal = None
            layout = l_tabs
    ...
    def build(self, tabs):
        ...
        for t in tabs:
            ...
            gs.update(g)
            dic[n] = sg.Tab(
                n, l, background_color=self.background_color, key=f"{n} TAB"
            )
            ls.append(dic[n])
        l_tabs = sg.TabGroup(
            [ls], key="ACTIVE_TAB", tab_location="topleft", **gui_aux.tab_kws
        )
    ...
    def get_vis_kwargs(self, v, **kwargs):
        ...
        w = self.window
        return (
            c["visualization"].get_dict(v, w)
            if "visualization" in c.keys()
            else screen.ScreenOps(**kwargs).nestedConf
        )
    ...
    def get_replay_kwargs(self, v):
        ...
        w = self.window
        return (
            c["Replay"].get_dict(v, w)
            if "Replay" in c.keys()
            else reg.generators.ReplayConf().nestedConf
        )
...
def speed_test():
    ...
    df = pd.DataFrame(res)
    df.to_csv(
        f"{ROOT_DIR}/gui/gui_speed_test.csv",
        index=0,
        header=["Tabs", "Open", "Close"],
    )
