...
class EssayTab(gui_aux.GuiTab):
    ...
    def build(self):
        kws = {
            "list_size": (25, 15),
            "canvas_size": gui_aux.col_size(x_frac=0.5, y_frac=0.4),
            "tab": self,
        }
        s1 = gui_aux.PadDict(
            "Essay",
        ...
        sl1 = gui_aux.SelectionList(
            tab=self, buttons=["load", "save", "delete", "exec"]
        )
        ...
        g2 = gui_aux.ButtonGraphList(
            self.exp_figures_key,
            list_header="Observed",
            fig_dict={},
            **kws,
            buttons=["browse_figs"],
        ...
        l = gui_aux.gui_cols(
            cols=[[sl1, s1, dl1], [g1.canvas, g2.canvas], [g1, g2]],
            x_fracs=[0.2, 0.55, 0.25],
            as_pane=True,
            pad=(20, 10),
        )
        return (
    ...
    def update(self, w, c, conf, id):
        ...
        essay = util.AttrDict(
            essay_ID=f'{id}_{reg.config.next_idx(id=id, conftype="Essay")}',
            path=f"essays/{id}",
        )
    ...
    def run_essay_exp(self, v, w, c, d, g, essay_exp):
        ...
        ds0 = essay.run()
        essay.anal()
        self.base_dict[essay_exp] = {
            "exp_fig_dict": essay.figs,
