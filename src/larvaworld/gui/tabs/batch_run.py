...
class BatchTab(gui_aux.GuiTab):
    ...
    def update(self, w, c, conf, id):
        w.Element(self.batch_id_key).Update(
            value=f'{id}_{reg.config.next_idx(id=id, conftype="Batch")}'
        )
    ...
    def get(self, w, v, c, **kwargs):
        ...
        conf = util.AttrDict(
            exp_kws={
                ...
                "experiment": self.current_conf(v)["exp_kws"]["experiment"],
            },
    ...
    def build(self):
        ...
        sl2 = gui_aux.SelectionList(
            tab=self, buttons=["load", "save", "delete", "exec"], sublists={"exp": sl1}
        )
        batch_conf = [
            [
                sg.T("Batch id:", **gui_aux.t_kws(8)),
                sg.In("unnamed_batch_0", k=self.batch_id_key, **gui_aux.t_kws(16)),
            ],
        ]
        s0 = gui_aux.PadDict(
            f"{self.name}_CONFIGURATION",
            content=batch_conf,
            disp_name="Configuration",
            dict_name="batch_setup",
            **kws,
        )
        s2 = gui_aux.PadDict("optimization", toggle=True, disabled=True, **kws)
        s3 = gui_aux.CollapsibleTable(
            "space_search",
            index="Parameter",
            heading_dict={"Range": "range", "N": "Ngrid"},
            dict_name="space_search_par",
            state=True,
            col_widths=[12, 8, 4],
            num_rows=5,
        )
        g1 = gui_aux.GraphList(
            self.name, tab=self, canvas_size=gui_aux.col_size(0.5, 0.8)
        )
        dl1 = gui_aux.DataList(
            ...
            disp="Stored batch-runs",
            size=(gui_aux.w_list, 5),
        )
        dl2 = gui_aux.DataList(
            ...
            disp="Active batch-runs",
            size=(gui_aux.w_list, 5),
        )

        l = gui_aux.gui_cols(
            cols=[[sl2, sl1, dl2, dl1, g1], [s0, s2, s3], [g1.canvas]],
            x_fracs=[0.20, 0.22, 0.55],
            as_pane=True,
            pad=(20, 10),
        )
        c = {}
    ...
    def run(self, v, w, c, d, g, conf, id):
        ...
        conf["experiment"] = id
        exec = sim.Exec(
            mode="batch", conf=conf, run_externally=self.gui.run_externally["batch"]
        )
...
def stored_trajs(experiment):
    path = f"{BATCH_DIR}/{experiment}/{experiment}.hdf5"
    try:
...
def retrieve_results(experiment, id):
    # from .. import reg
    f = f"{SIM_DIR}/batch_runs/{experiment}/{id}/results.h5"
    try:
...
def delete_traj(experiment, key):
    # from .. import reg
    path = f"{SIM_DIR}/batch_runs/{experiment}/{experiment}.hdf5"
