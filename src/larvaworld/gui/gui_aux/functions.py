    ...
    **kwargs,
    ...
    for e, x in zip(element_list, x_fracs):
        if not as_pane:
            ...
            try:
                ll = sg.Col(
                    e.get_layout(as_pane=True),
                    **col_kws,
                    size=col_size(x_frac=x, y_frac=y_frac),
                    **kwargs,
                )
            except:
                ll = sg.Col(
                    [[sg.Pane([sg.Col(e)], pad=pad, border_width=8)]],
                    **col_kws,
                    size=col_size(x_frac=x, y_frac=y_frac),
                    **kwargs,
                )
    ...
    for i, e in enumerate(element_list):
        if type(e) == list:
            if all([type(ee) != list for ee in e]):
                ...
                e = gui_rowNcol(
                    e, x_fracs=x_fracs[i], y_fracs=y_fracs[i], as_pane=as_pane
                )
            l.append(e)
        else:
