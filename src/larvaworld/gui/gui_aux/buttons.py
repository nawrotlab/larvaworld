...
def button_row(name, buttons, button_args={}):
    but_tips = {
        ...
        "tree": "Inspect the default configuration parameter tree.",
        "conf_tree": "Inspect the specific configuration parameter tree.",
    }
    png_dict = {
        "settings": "developer-tools",
        "tree": "kinesis-data-streams",
        "conf_tree": "kinesis-data-firehose",
    }
    but_kws = {
        "browse": {
            "initial_folder": DATA_DIR,
            "enable_events": True,
            "target": (0, -1),
            "button_type": sg.BUTTON_TYPE_BROWSE_FOLDER,
        },
        "browse_figs": {
            "initial_folder": f"{ROOT_DIR}/gui/media/exp_figures",
            "enable_events": True,
            "target": (0, -1),
        ...
        if from_bs64:
            ...
            for i in range(3):
                try:
                    ...
                    filename = f"{ROOT_DIR}/gui/media/icons/icons{i+1}/{name}.png"
                    super().__init__(
...
class BoolButton(sg.Button):
    def __init__(self, name, state, disabled=False, **kwargs):
        ...
        super().__init__(
            image_data=self.get_image(self.state, self.disabled),
            k=f"TOGGLE_{self.name}",
            metadata=BtnInfo(state=self.state),
            **gui_aux.b6_kws,
            **b_kws,
            **kwargs,
        )
...
class ClickableImage(sg.Button):
    def __init__(self, name, link, **kwargs):
        ...
        self.link = link
        super().__init__(
            key=f"ClickableImage {self.name}",
            enable_events=True,
            metadata=BtnLink(link=self.link),
            **kwargs,
        )
...
def color_pick_layout(name, color=None, show_text=False):
    t = (
        [sg.T("", **gui_aux.t_kws(5)), sg.T("color", **gui_aux.t_kws(5))]
        if show_text
        else []
    )
    return [
        *t,
        ...
        GraphButton(
            "Button_Color_Circle",
            f"PICK {name}_color",
            button_type=sg.BUTTON_TYPE_COLOR_CHOOSER,
