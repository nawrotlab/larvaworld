...
class SectionDict:
    def __init__(
        self, name, dict, type_dict=None, toggled_subsections=True, value_kws={}
    ):
        self.init_dict = dict
        ...
        for k, v in self.init_dict.items():
            ...
            if type(v) == bool:
                ...
                if d is not None:
                    ...
                    if t == list:
                        if all(
                            [type(i) in [int, float] for i in t0 if i not in ["", None]]
                        ):
                            temp = sg.Spin(
                                values=t0, initial_value=v, key=k0, **value_kws
                            )
                        else:
                            temp = sg.Combo(
                                t0,
    ...
    def get_dict(self, v, w):
        ...
        for k, v0 in d.items():
            ...
            elif type(t) == tuple or (
                type(t) == dict and list(t.keys()) == ["type", "values"]
            ):
                d[k] = w[k0].get()
...
class MultiSpin(sg.Pane):
    def __init__(
        ...
        **kwargs,
        ...
        add_button = gui_aux.GraphButton(
            "Button_Add", self.add_key, tooltip="Add another item in the list."
        )
        remove_button = gui_aux.GraphButton(
            "Button_Remove", self.remove_key, tooltip="Remove last item in the list."
        )
    ...
    def build_spins(self):
        ...
        spins = [
            func(initial_value=vv, key=kk, visible=vis, **spin_kws)
            for vv, kk, vis in zip(self.v_spins, self.k_spins, self.visibles)
        ]
        return spins
    ...
    def build_layout(self):
        ...
        if Ng is not None and self.Nspins >= Ng:
            spins = util.SuperList(ss).group_by_n(Ng)
            spins = [sg.Col(spins)]
            return spins
        else:
            return ss
...
class ProgressBarLayout:
    def __init__(self, list, size=(8.8, 20)):
        ...
        self.l = [
            sg.Text("Progress :", **gui_aux.t_kws(8)),
            sg.ProgressBar(
                100,
                orientation="h",
                size=size,
                key=self.k,
                bar_color=("green", "lightgrey"),
                border_width=3,
            ),
            gui_aux.GraphButton(
                "Button_Check",
...
class SelectionList(GuiElement):
    def __init__(
        ...
        **kwargs,
        ...
        if disp is None:
            disps = [
                k for k, v in self.tab.gui.tab_dict.items() if v[1] == self.conftype
            ]
            if len(disps) == 1:
                disp = disps[0]
            elif len(disps) > 1:
                raise ValueError(
                    "Each selectionList is associated with a single configuration type"
                )
        self.disp = disp

        self.progressbar = (
            ProgressBarLayout(self, size=(self.width - 16, 20)) if progress else None
        )
    ...
    def update(self, w, id="", all=False, values=None):
        if values is None:
            values = self.confs
        w.Element(self.k).Update(
            values=values, value=id, size=(self.width, self.Nconfs)
        )
    ...
    def get_next(self, k0):
        ...
        idx = (
            int(np.min([i for i in range(5) if f"{k0}{i}" not in w.AllKeysDict.keys()]))
            if w is not None
            else 0
        )
        return f"{k0}{idx}"
...
class Header(HeadedElement):
    def __init__(self, name, content=[], text=None, single_line=True, **kwargs):
        ...
        super().__init__(
            name=name, header=header, content=content, single_line=single_line
        )
        ...
        if drop_down:
            ...
            if self.aux_cols is None:
                ...
                l = [
                    Table(
                        values=vs,
                        headings=["Dataset ID"] + self.aux_cols,
...
class DataList(NamedList):
    def __init__(
        ...
        tab,
        dict={},
        buttons=["select_all", "remove", "changeID", "browse"],
        button_args={},
        raw=False,
    ...
    def eval(self, e, v, w, c, d, g):
        ...
        v0 = v[k]
        kks = [
            v0[i] if self.aux_cols is None else list(d0.keys())[v0[i]]
        ...
        if e == self.browse_key:
            ...
            if len(v0) > 0:
                d0[kks[0]].visualize(
                    vis_kwargs=self.tab.gui.get_vis_kwargs(v, mode="video"),
            ...
            if len(v0) > 0:
                if d0[kks[0]].config["refID"] is None:
                    gui_aux.save_ref_window(d0[kks[0]])
                exp_conf = reg.generators.ExpConf.imitation_exp(
                    refID=d0[kks[0]].config["refID"]
                ).nestedConf
...
class Collapsible(HeadedElement, GuiElement):
    def __init__(
        ...
        header_dict=None,
        header_value=None,
        header_list_width=10,
        header_list_kws={},
        header_text_kws=gui_aux.t_kws(12),
        header_key=None,
        use_header=True,
        Ncols=1,
        col_idx=None,
        **kwargs,
        ...
        if use_header:
            ...
            after_header = (
                [gui_aux.BoolButton(name, toggle, disabled)]
            ...
            header_kws = {
                "text": disp_name,
                "text_kws": header_text_kws,
                "before_header": [self.sec_symbol],
            ...
            if header_dict is None:
                ...
                header_l = NamedList(
                    name,
                    choices=list(header_dict.keys()),
                    default_value=header_value,
                    key=self.header_key,
                    size=(header_list_width, None),
                    list_kws=header_list_kws,
                    header_kws=header_kws,
                )
            header = header_l.get_layout()
            HeadedElement.__init__(
                self,
                name=name,
                header=header,
    ...
    def get_symbol(self):
        return sg.T(
            SYMBOL_DOWN if self.state else SYMBOL_UP,
            k=f"OPEN {self.sec_key}",
            enable_events=True,
            text_color="black",
            **gui_aux.t_kws(2),
        )
    ...
    def arrange(self, content, Ncols=1, col_idx=None):
        if col_idx is not None:
            ...
            content = util.group_list_by_n(
                [*content], int(np.ceil(len(content) / Ncols))
            )
            content = [[sg.Col(ii, **gui_aux.col_kws) for ii in content]]
        return content
...
class CollapsibleTable(Collapsible):
        ...
        **kwargs,
        ...
        content = [
            [
                Table(
                    values=self.data,
                    headings=[index] + self.headings,
                    def_col_width=7,
                    key=self.key,
                    num_rows=max([num_rows, len(self.data)]),
                    col_widths=col_widths,
                    visible_column_map=col_visible,
                )
            ]
        ]
    ...
    def update(self, w, dic=None, use_prefix=True):
        ...
        if self.color_idx is not None:
            ...
            row_cols = None
        w[self.key].update(
            values=self.data, num_rows=len(self.data), row_colors=row_cols
        )
        if self.auto_open:
...
def combo_layout(name, title, dic, **kwargs):
    ...
    for k, args in dic.items():
        kws = {"values": args["values"], "key": f"{name}_{k}", **kwargs}
        spin_kws = {
        ...
        if disp in ["initial", "mean"]:
            ...
            d["r"] = [
                sg.T(f"{disp}:", **gui_aux.t_kws(5)),
                MultiSpin(**spin_kws, Nspins=2),
            ]
        elif disp in ["name"]:
            d["name"] = [
                sg.T(f"{title}:", **gui_aux.t_kws(6)),
                sg.Combo(
                    **kws,
                    default_value=args["initial_value"],
                    enable_events=True,
                    readonly=True,
                    **gui_aux.t_kws(10),
                ),
...
class CollapsibleDict(Collapsible):
    def __init__(
        ...
        **kwargs,
        ...
        if type_dict is None:
            ...
            entry = (
                par(name=as_entry, dtype=str, v="Unnamed")
    ...
    def build(self, name, type_dict, value_kws={}, text_kws={}):
        ...
        for k, args in type_dict.items():
            k0 = f"{name}_{k}"
            if args["dtype"] == dict:
                subdicts[k0] = CollapsibleDict(
                    k0,
                    disp_name=k,
                    type_dict=args["content"],
                    text_kws=text_kws,
                    value_kws=value_kws,
                    state=self.subdict_state,
                )
...
class PadDict(PadElement):
    def __init__(
        ...
        **kwargs,
        ...
        self.content = self.arrange_content(
            content, col_idx=col_idx, row_idx=row_idx, Ncols=Ncols
        )
        self.dtypes = (
            {k: type_dict[k]["dtype"] for k in type_dict.keys()}
    ...
    def arrange_content(self, content, col_idx=None, row_idx=None, Ncols=1):
        ...
        if col_idx is not None:
            ...
            content = util.SuperList([*content]).group_by_n(
                int(np.ceil(len(content) / Ncols))
            )
            content = [[sg.Col(ii, **gui_aux.col_kws) for ii in content]]
        return content
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.content = self.build(
            col_widths=col_widths, num_rows=num_rows, col_visible=col_visible
        )
    ...
    def build(self, col_widths, num_rows, col_visible):
        ...
        return [
            [
                Table(
                    values=self.data,
                    headings=[self.index] + self.headings,
                    def_col_width=7,
                    key=self.key,
                    num_rows=max([num_rows, len(self.data)]),
                    col_widths=col_widths,
    ...
    def eval(self, e, v, w, c, d, g):
        ...
        if e == f"ADD {self.name}":
            if len(Ks) > 0:
                ...
                id = None
            entry = gui_aux.entry_window(
                id=id, base_dict=self.dict, index=self.index, dict_name=self.dict_name
            )
            self.dict.update(**entry)
            self.update(w)
        elif e == f"REMOVE {self.name}":
            ...
            tree = GuiTreeData(
                entries=entries, headings=[ids], col_widths=[40] + [20] * len(ids)
            )
...
class Table(sg.Table):
    def __init__(
        ...
        header_background_color="lightgrey",
...
class GraphList(NamedList):
    def __init__(
        ...
        list_header="Graphs",
        auto_eval=True,
        canvas_kws={"background_color": "Lightblue"},
        graph=False,
        subsample=1,
        **kwargs,
    ):
        ...
        self.canvas, self.canvas_element = self.init_canvas(
            canvas_size, canvas_kws, graph
        )
        self.fig_agg = None
...
class ButtonGraphList(GraphList):
    def __init__(
        self,
        name,
        buttons=["refresh_figs", "conf_fig", "draw_fig", "save_fig"],
        button_args={},
        **kwargs,
    ):
        super().__init__(
            name=name,
            next_to_header=gui_aux.button_row(name, buttons, button_args),
            **kwargs,
        )
    ...
    def generate(self, w, data):
        if self.func is not None and len(list(data.keys())) > 0:
            try:
                self.fig, self.save_to, self.save_as = self.func(
                    datasets=list(data.values()), return_fig=True, **self.func_kws
                )
    ...
    def refresh_figs(self, w, data):
        ...
        if len(data) > 0:
            ...
            for i, (name, func) in enumerate(self.fig_dict.items()):
                ...
                w.refresh()
                try:
                    fig, save_to, save_as = func(
                        datasets=list(data.values()),
                        labels=list(data.keys()),
                        return_fig=True,
                        **self.get_graph_kws(func),
                    )
                    valid.append(name)
                except:
                    pass
    ...
    def save_fig(self):
        ...
        if self.fig is not None:
            l = [
                [
                    sg.T("Filename", **gui_aux.t_kws(10)),
                    sg.In(default_text=self.save_as, k=kDir, **gui_aux.t_kws(80)),
                ],
                [
                    sg.T("Directory", **gui_aux.t_kws(10)),
                    sg.In(self.save_to, k=kFil, **gui_aux.t_kws(80)),
                    sg.FolderBrowse(
                        initial_folder=ROOT_DIR, key=kFil, change_submits=True
                    ),
                ],
                [sg.Ok(), sg.Cancel()],
            ]
    ...
    def eval(self, e, v, w, c, d, g):
        ...
        if e == f"BROWSE_FIGS {n}":
            ...
            if self.func_kws != {}:
                self.func_kws = gui_aux.set_kwargs(
                    self.func_kws, title="Graph arguments"
                )
        elif e == f"DRAW_FIG {n}":
...
class DynamicGraph:
    def __init__(self, agent, pars=[], available_pars=None):
        ...
        self.window = sg.Window(
            f"{self.agent.unique_id} Dynamic Graph",
            self.layout,
            finalize=True,
            location=(0, 0),
            size=(W, H),
    ...
    def build_layout(self):
        ...
        l0 = [
            [sg.T("Choose parameters")],
            [
                sg.Col(
                    [
                        *[
                            [sg.CB(p, k=f"k_{p}", **gui_aux.t_kws(24))]
                            for p in par_lists[i]
                        ]
                    ]
                )
    ...
    def update_pars(self):
        ...
        self.pars, syms, us, lims, pcs = reg.getPar(
            d=self.pars, to_return=["d", "disp", "l", "lim", "p"]
        )
        self.Npars = len(self.pars)
        self.yranges = {}
        self.fig, axs = plt.subplots(
            self.Npars, 1, figsize=self.figsize, dpi=self.my_dpi, sharex=True
        )
        self.axs = axs.ravel() if self.Npars > 1 else [axs]
        Nticks = int(self.init_dur / self.dt)
        for i, (ax, p, l, u, lim, p_col) in enumerate(
            zip(self.axs, self.pars, syms, us, lims, pcs)
        ):
        ...
        self.fig.subplots_adjust(
            bottom=0.15, top=0.95, left=0.1, right=0.99, wspace=0.01, hspace=0.05
        )
        if self.fig_agg:
            delete_figure_agg(self.fig_agg)
...
class GuiTreeData(sg.TreeData):
    ...
    def _NodeStr(self, node=None, level=1, k="name", v="description"):
        ...
        return "\n".join(
            [str_pair(node, k, v)]
            + [
                " " * 4 * level + self._NodeStr(child, level + 1, k, v)
    ...
    def save(self, **kwargs):
        with open(f"{CONF_DIR}/glossary.txt", "w") as f:
    ...
    def test(self):
        w = sg.Window(
            "Parameter tree", self.build_layout(), size=(self.w_width * 20, 800)
        )
        while True:
            e, v = w.read()
            if e == "Ok":
...
def detect_dataset(labID=None, path=None, raw=True, **kwargs):
    ...
    if raw:
        ...
        if dFp is not None:
            ...
            ids, dirs = (
                [f.split(df_)[1:][0] for f in os.listdir(path) if f.startswith(dfp)],
                [path],
            )
            for id, dr in zip(ids, dirs):
                dic[id] = dr
            ...
            ids = util.unique_list(
                [f.split(df_)[0] for f in os.listdir(path) if df_ in f]
            )
            for id in ids:
                dic[id] = path
        return dic
